...
- (unsigned int)crc16
{
	...
	// (note that for instance XModem and ZModem use a different calculation
	// for their CRCs!)
...
/*
 * The following method was derived from a
 * universal CRC (16-64 bit) routine
 * by Matthew Dillon, email@hidden. For more information, see:
 * http://www.backplane.com/diablo/crc64.html
 *
 * Thomas Tempelmann) modified the code so that it uses
 * CW's 64 Bit type directly instead of dealing with two 32 bit ints.
 *
 * Note that calculating a 16 or 32 bit wide CRC using this routine
 * does not produce the same result as the other fixed-width CRC routines,
